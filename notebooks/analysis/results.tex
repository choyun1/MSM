...
\begin{tikzpicture}
  % Title
  \node[draw=cyan, text=cyan, thick, rounded corners, opacity=1] at (3*pi/4, 2) {\textbf{Experiment EV} ($\Delta \theta_{\textrm{\tiny RMS}} = 127^{\circ}$)};
...
\begin{tikzpicture}
  % Title
  \node[draw=violet, text=violet, thick, rounded corners, opacity=1] at (3*pi/4, 2) {\textbf{Experiment DV} ($\Delta \theta_{\textrm{\tiny RMS}} = 90^{\circ}$)};
